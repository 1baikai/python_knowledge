...
class HTTPServer(object):
    ...
    def server_forever(self):
        ...
        print('Listent the port %d'%self.port)
        while True:
            try:
                ...
                continue
            #创建新的线程处理请求
            clientThread = Thread(target = self.handleRequst,args=(connfd,))
            clientThread.setDaemon(True)
            clientThread.start()
    #客户端请求函数
    def handleRequst(self,connfd):
        #接收客户端请求
    ...
    #     if getRequest == '/':
    #         filename =self.static_dir + '/index.html'
    #         file_list = os.listdir(self.static_dir)
    #         print(file_list)
